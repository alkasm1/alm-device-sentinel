...
console.log("ALM Device Sentinel - Change Detection v0.6\n");

const networkDiscovery = new NetworkDiscovery();
const nmapDiscovery = new NmapDiscovery();
...
async function processDiscovery(discovery, label) {
  console.log(label);
  console.log("=".repeat(label.length));

  for (const found of discovery.devices) {
    const result = deviceManager.upsert({
      identity: {
        mac: found.mac,
      ...
      status: {
        online: found.reachable
      }
    });

    const events = changeDetector.detect(
      result.previous,
      result.device,
      result.created
    );
    ...
    const risk = riskEngine.calculate(events);

    result.device.risk = risk;
...
async function main() {
  const networkInfo = await networkDiscovery.discover();

  console.log("Network Discovery");
  console.log("-----------------");

  if (!networkInfo.success) {
    console.error("Failed:", networkInfo.error);
    process.exitCode = 1;
    return;
  }

  console.log(`Interface : ${networkInfo.localInterface}`);
  ...
  console.log(`CIDR              : ${discoveryResult.network.cidr}`);
  ...
  if (discoveryResult.limitations?.length) {
    console.log("\nLimitations:");
    
    for (const limitation of discoveryResult.limitations) {
  ...
  if (devices.length > 0) {
    ...
    const result = deviceManager.updateKnownDevice(
      device.id,
      simulatedState
    );
  ...
  const knownDevices = deviceManager.getAll();

  if (knownDevices.length > 0) {
    const device = knownDevices[0];

    console.log("\nSTATE SIMULATION");
    console.log("================");

    // Simulate device going offline
    const offlineResult = deviceManager.updateKnownDevice(
      device.id,
      {
        status: {
          online: false
    ...
    const offlineEvents = changeDetector.detect(
      offlineResult.previous,
      offlineResult.device,
      false
    );

    eventStore.addMany(offlineEvents);
    ...
    const onlineResult = deviceManager.updateKnownDevice(
      device.id,
      {
        status: {
          online: true
    ...
    const onlineEvents = changeDetector.detect(
      onlineResult.previous,
      onlineResult.device,
      false
    );

    eventStore.addMany(onlineEvents);
    ...
    const stateRisk = riskEngine.calculate(stateEvents);

    onlineResult.device.risk = stateRisk;
  ...
  for (const device of deviceManager.getAll()) {
    console.log(
      JSON.stringify(
        {
          id: device.id,
          mac: device.identity.mac,
          hostname: device.identity.hostname,
          ipv4: device.network.ipv4,
          online: device.status.online,
          risk: device.risk,
          firstSeen: device.status.firstSeen,
